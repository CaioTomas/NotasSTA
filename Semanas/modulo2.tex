...
\section{A função Gama}
    ...
    associa $n! = n \cdot (n-1) \cdots 3 \cdot 2 \cdot 1$ e convenciona-se $0! = 1$. 
    ...
    matemática e que pode ser vista como uma extensão do conceito de fatorial. 
    Para todo $s>0$, definimos
    %
    \[
    ...
    \begin{align*}
        ...
        &\leq \int_{\e}^{1}t^{s-1} \, dt \\
        &= \frac{t^s}{s}\Big |_{\e}^{1} \\
    ...
    %
    \begin{align*}
        \int_{\e}^{1}e^{-t}t^{s-1} \, dt &= \int_{\e}^{1}e^{-t}t^{-1-\delta} \, dt \\
        &\geq \int_{\e}^{1}t^{-1-\delta} \, dt \\
        &= \frac{t^{-\delta}}{\delta}\Big |_{t = \e}^{1} \\
    ...
    %
    \begin{align*}
        \int_{1}^{1/\e}e^{-t}t^{s-1} \, dt &\leq M\int_{\e}^{1}e^{-t/2} \, dt \\
        &= 2M(e^{-1/2} - e^{-1/2\e}) < \infty
    \end{align*}
    %
    para todo $\e > 0$. Estas considerações mostram que a função $\Gamma$ está bem 
    ...
    %
    \begin{lema}
        Sejam $\Omega \subseteq \C$ um domínio e $f: U \times \R_{\geq 0}$. 
        Suponha que, para cada $z \in U$, a aplicação $t \mapsto f(z,t)$ seja 
        Riemann integrável e que existe 
        %
        \[
    ...
    %
    \begin{observacao}
        No lema anterior, assumimos que a função $f$ com $z$ fixado é Riemann integrável 
        em relação à variável $t$ real. Esta hipótese é equivalente a assumir que o 
        conjunto de descontinuidades da aplicação $t \mapsto f(z,t)$ tem medida de Lebegue nula. 
        Além disso, vale um resultado análogo ao anterior: Se assumimos que existe 
        %
        \[
    ...
    %
    \begin{align*}
        \int_{0}^{\infty}|e^{-t}t^{s-1}| \, dt &= \int_{0}^{\infty}e^{-t}|t^{s-1}| \, dt
        %
        &= \int_{0}^{\infty}e^{-t}t^{\sigma-1} \, dt = \Gamma(\sigma)
    \end{align*}
    %
    sempre que $\sigma > 0$. O Lema anterior mostra que, neste caso, existe 
    %
    $$\int_{0}^{\infty}e^{-t}t^{s-1} \, dt.$$
    %
    \begin{teorema}
    ...
    \begin{proof}
        ...
        Aqui, por abuso de linguagem, denotaremos esta candidata por $\Gamma(s)$ também. Seja 
        $0 < \delta < M$ e $S = S_{\delta, M} = \{ s \in \C : \delta < \Re{s} < M\}$. 
        Vamos mostrar que, em toda faixa deste tipo, $\Gamma(s)$ é o limite da sequência 
        ...
        \]
        %
        Sendo $\Delta$ um triângulo contido em $S$, podemos aplicar os Teoremas de 
        Morera e Fubini em cada $g_n$ e obter
        %
        \begin{align*}
        ...
        \end{align*}
        %
        Isto mostra que cada $g_n$ é de fato holomorfa.
        %
        \begin{align*}
            ...
            %
            &\leq \frac{1}{\delta}\frac{1}{n^\delta} + K_M\int_{n}^{\infty}e^{-t/2} \, dt \to 0
        \end{align*}
        %
        conforme $n \to \infty$. Além disso, esta convergência é uniforme, isto mostra que $\Gamma(s)$ é holomorfa em cada faixa $S$ \textcolor{red}{(citar teorema)}. Como $M$ e $\delta$ são arbitrários, segue que $\Gamma(s)$ é holomorfa em $U$.
    \end{proof}
    %
    ...
    %
    Se $\Re \ s >0$, podemos integral ambos os lados e obter
    %
    \begin{align*}
        0 &= \lim_{\e \to 0^+} e^{-t}t^s \big |_{\e}^{1/\e} \\
        %
    ...
    \end{align*}
    %
    Portanto, fica demonstrado a seguinte proposição.
    
    \begin{proposicao}
    Se $\Re \ s>0$, então $\Gamma(s+1) = s\Gamma(s)$.
    \end{proposicao}
    
    É fácil, usando esta proposição, verificar a afirmação feita no início desta seção, isto é, $\Gamma(n) = (n-1)!$ para todo $n > 0$ inteiro. 
    
    \begin{teorema}
    A função $\Gamma$, definida para $\Re \ s > 0$, admite uma continuação analítica para uma função meromorfa em $\C$ com polos simples nos inteiros negativos.
    \end{teorema}
    \begin{proof}
