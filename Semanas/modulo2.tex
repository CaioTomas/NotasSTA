...
\section{A função Gama}
    ...
    \end{equation}
    %
    Vamos mostrar que a integral à direita define uma função inteira. Seja $M > 0$ e defina $S_M = \{s \in \C : |\Re(s)| < M\}$. Defina também, para cada $n \in \N$,
    %
    \[
    g_n(s) = \int_{1}^{n}e^{-t}t^{s-1} \, dt
    \]
    %
    para $s \in S_M$. Vamos usar o Teorema de Morera para mostrar que cada $g_n$ é holomorfa e vamos verificar que a sequência definida por estas funções converge uniformemente para a integral à direita em (\ref{eq-Gamma1}). 
    
    Sejam $s,z \in S_M$, então
    %
    \begin{align*}
        |g_n(s) - g_n(z)| &= \Big | \int_{1}^{n}e^{-t}(t^{s-1}-t^{z-1}) \, dt\Big | \\
        %
        &\leq \int_{1}^{n}\frac{e^{-t}}{t}|t^{s}-t^{z}| \, dt \\
        %
        &\leq \int_{1}^{n}|t^{s}-t^{z}| \, dt 
    \end{align*}
    %
    Observe que a função $s \mapsto t^s = \exp(s\ln t)$ é inteira, então, dado $\e > 0$, podemos encontrar $\delta > 0$ tal que $|t^{s}-t^z| < \e$ se $|s-z| < \delta$. Segue que 
    %
    \begin{align*}
    |g_n(s) - g_n(z)| &\leq \int_{1}^{n}|t^{s}-t^{z}| \, dt \\
    %
    &\leq \e \int_{1}^{n} \, dt = (n-1)\e \xrightarrow{\e \to 0} 0.
    \end{align*}
    %
    Isto mostra que as funções $g_n$ são contínuas em cada faixa $S_M$. Como $M$ é arbitrário, segue que são contínuas em $\C$. Se $\Delta$ é um triângulo totalmente contido em $S_M$, temos
    %
    \begin{align*}
    ...
    \end{align*}
    %
    Pelo Teorema de Morera, as $g_n$ são holomorfas e, se estendermos sua faixa de definição indefinidamente, vemos que definem funções inteiras.
    
    Observe agora que 
    %
    \begin{align*}
        \Big | \int_{1}^{\infty}e^{-t}t^{s-1} \, dt - \int_{1}^{n}e^{-t}t^{s-1} \, dt \Big | &= \Big | \int_{n}^{\infty}e^{-t}t^{s-1} \, dt \Big | \\
        %
        &\leq \int_{n}^{\infty}e^{-t}|t^{s-1}| \, dt \\
        %
        &= \int_{n}^{\infty}e^{-t}t^{\Re(s)-1} \, dt.
    \end{align*}
    %
    Esta é a cauda da integral que define $\Gamma(\Re(s))$, logo, deve ir a zero conforme $n \to \infty$. Portanto,
    %
    \[
