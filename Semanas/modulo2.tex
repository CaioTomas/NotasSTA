...
\section{A função Gama}
    ...
    \]
    %
    Nesta seção, vamos estudar a função Gama, $\Gamma$, uma função de grande importância na Análise 
    ...
    \begin{center}
    {\red 
        O lema abaixo precisa ser revisto.\\ 
        Acredito que ele pode ser reescrito de modo a ser aplicável à Gama.
    }
    \end{center}
    %%
    \begin{lema}
        Sejam $\Omega \subseteq \C$ um domínio e $f: \Omega \times \R_{\geq 0}\to \C$. 
        Suponha que, para cada $z \in \Omega$, a aplicação $t \mapsto f(z,t)$ tenha um conjunto descontinuidades com medida de Lebesgue nula. Além disso, para cada $a>0$, temos 
        %
        $$\sup_{t \in [0,a]} |f(z,t)| < \infty.$$
        %
        Se existe 
        %
        \[
        \lim_{a \to \infty} \int_{0}^{a}|f(z,t)| \, dt,
        \]
        %
        então existe 
        %
        \[
        g(z) \equiv \lim_{a \to \infty} \int_{0}^{a}f(z,t) \, dt.
        \]
        %
    \end{lema}
    %
    \begin{proof}
        Com as hipóteses que temos, está bem definida, para todo $a>0$, a integral
        %
        \[
        \int_{0}^{a}|f(z,t)| \, dt.
        \]
        %
        Além disso, existe o limite 
        %
        \[
        I = \lim_{a \to \infty} \int_{0}^{a}|f(z,t)| \, dt.
        \]
        % 
        Isto significa que, dado $\e > 0$ qualquer, existe $M > 0$ tal que, se $b > M$, temos
        %
        \[
        \Big | I - \int_{0}^{b}|f(z,t)| \, dt \Big | < \e.
        \]
        %
        Usando a segunda desigualdade triangular com $c > b > M$, temos
        %
        \begin{align*}
            \left| I - \int_{0}^{b}|f(z,t)| \, dt \right| 
            &= \left| I - \int_{0}^{c}|f(z,t)| \, dt + \int_{0}^{c}|f(z,t)| \, dt 
            - \int_{0}^{b}|f(z,t)| \, dt \right| \\
            &\geq \left| \int_{0}^{c}|f(z,t)| \, dt - \int_{0}^{b}|f(z,t)| \, dt \right| 
            - \left| I - \int_{0}^{c}|f(z,t)| \, dt \right| \\
            %
            &= \left| \int_{b}^{c}|f(z,t)| \, dt \right| 
            - \left| I - \int_{0}^{c}|f(z,t)| \, dt \right|.
        \end{align*}
        %
        Logo, 
        %
        \[
        \left| \int_{b}^{c}|f(z,t)| \, dt \right| < 2\e.
        \]
        %
        Seja $\{a_n: n \in \N \}$ uma sequência de números reais positivos tal que 
        $a_n \xrightarrow{n\to\infty} \infty$ e defina
        %
        \[
        I_n  \equiv I_n(z) = \lim_{a \to \infty} \int_{0}^{a}f(z,t) \, dt.
        \]
        %
        Os $I_n$ definem uma sequência de números complexos. Para mostrar que esta 
        sequência converge, mostramos que ela é de Cauchy.
        %
        \begin{align*}
            |I_n - I_m| 
            &= \left| \int_{0}^{a_n}f(z,t) \, dt - \int_{0}^{a_n}f(z,t) \, dt \right|\\
            &= \left| \int_{c_{n,m}}^{d_{n,m}}f(z,t) \, dt \right| \\
            &\leq \int_{c_{n,m}}^{d_{n,m}}|f(z,t)| \, dt \xrightarrow{n,m \to \infty} 0
        \end{align*}
        %
        pelo que vimos no inicio da demonstração. Aqui, denotamos 
        $c_{n,m} = \min\{a_n,a_m\}$  e $d_{n,m} = \max\{a_n,a_m\}$. 
        Então esta sequência é de Cauchy e deve convergir para um limite $J$ que 
        pode depender da sequência $a_n$. Vamos mostrar que este limite é o 
        mesmo para qualquer sequência. Sejam $J_1, J_2$ os limites obtidos usando 
        sequências $a_n$ e $b_n$ respectivamente. Observe que
        %
        \begin{align*}
            |J_1 - J_2| 
            &= \lim_{n \to \infty} \left| \int_{0}^{a_n}f(z,t) \, dt 
            - \int_{0}^{b_n}f(z,t) \, dt \right| \\
            &= \lim_{n \to \infty} \left| \int_{c_n}^{d_n}f(z,t) \, dt \right| \to 0
        \end{align*}
        %
        conforme o que já mostramos. Aqui, $c_n = \min\{a_n,b_n\}$ e $d_n = \max\{a_n,b_n\}$. 
        Isto implica que $J_1 = J_2$.
    \end{proof}
    %
    \begin{observacao}
    ...
    Seja $s \in \C$ e denote $\sigma = \Re \ s$, temos, para $t>0$,
    %
    \begin{align*}
        |t^{s-1}| &= |\exp((s-1)\ln t)| \\
        %
    ...
    %
    \begin{equation*}
        \int_{0}^{\infty}|e^{-t}t^{s-1}| \, dt = \int_{0}^{\infty}e^{-t}|t^{s-1}| \, dt = \int_{0}^{\infty}e^{-t}t^{\sigma-1} \, dt = \Gamma(\sigma)
    \end{equation*}
    %
    sempre que $\sigma > 0$. O Lema anterior mostra que, neste caso, existe
    para todo $s\in\C$ tal que $\Re(s)>0$ 
    %
    $$\Gamma(s)\equiv \int_{0}^{\infty}e^{-t}t^{s-1} \, dt.$$
    %
    \begin{teorema}
        A função $\Gamma$ admite uma extensão analítica para o semiplano 
        $U = \{s \in \C : \Re(s) >0 \}$.
    \end{teorema}
    %
    \begin{proof}
        ...
        para cada $n\in \N$ e concluir então que $\Gamma$ é holomorfa em na faixa $S$. 
        
        Para provar as afirmações feitas acima, vamos começar mostrando que para cada $n\in \N$ fixado que $g_n:S\to C$ é uma função contínua. De fato, dados quaisquer $s,\widetilde{s}\in S$ temos  
        \begin{align*}
            |g_n(s)-g_n(\widetilde{s})|
            &=
            \left|
            \int_{1/n}^{n}e^{-t}t^{s-1} \, dt
            -
            \int_{1/n}^{n}e^{-t}t^{\widetilde{s}-1} \, dt
            \right|
            \\[0.3cm]
            &\leqslant
            \int_{1/n}^{n}
            e^{-t} |t^{s-1}-t^{\widetilde{s}-1}|            
            \, dt
            \\[0.3cm]
            &\leqslant
            \int_{1/n}^{n}
            \frac{e^{-t}}{t} |t^{s}-t^{\widetilde{s}}|            
            \, dt
            \\[0.3cm]
            &\leqslant
            n
            \int_{1/n}^{n}
            |e^{s\ln t}-e^{\widetilde{s}\ln t}|            
            \, dt
            \\[0.3cm]
            &\leqslant
            n
            \int_{1/n}^{n}
            |s-\widetilde{s}| (\ln n)  e^{M\ln n}            
            \, dt
            \\[0.3cm]
            &\leqslant
            n^2(\ln n)  e^{M\ln n}|s-\widetilde{s}|.
        \end{align*}
    O que mostra que a função $g_n$ é Lipschitz e portanto contínua.
        
        
    Próximo passo é mostra que a integral de $g_n$ ao longo de qualquer
    ...
    \begin{teorema}
        A função $\Gamma$, definida para $\Re(s) > 0$, admite uma continuação analítica para uma função meromorfa em $\C$ com polos simples nos inteiros negativos. Além disso, o resíduo em $-n$ é $(-1)^{n}/n!$ para todo $n \in \N \cup \{0\}$.
    \end{teorema}
    \begin{proof}
    ...
    %
    então $0$ é um polo simples de $F_1$ com resíduo 1.
    
    Definimos agora a função holomorfa $F_2 : U_2 \to \C$ dada por
    %
    ...
    \end{align*}
    %
    o resíduo de $F_{n+1}$ em $-n$. 
    
    Podemos definir $\Gamma(s)$ por $F_n(s)$ em qualquer semiplano $U_n$ com polos simples nos inteiros negativos maiores que $-n$, ou seja, estendemos $\Gamma$ para uma função meromorfa em $\C$. Esta construção está bem definida pela unicidade das extensões analíticas.
    \end{proof}
    
    A equação funcional $s\Gamma(s) = \Gamma(s+1)$ é válida para para todo $s \in \C$ que não é polo de $\Gamma$. Para ver isso, basta notar que a função $f(s) = \Gamma(s+1) - s\Gamma(s)$ definida em $\C \setminus \{0, -1, -2, \dots\}$ é nula para $\Re \ s > 0$, logo, é identicamente nula pelo princípio da identidade. 
    
    Esta equação funcional é bem conhecida para a função $\Gamma$ real, então poderíamos usar este fato e ter procedido como fizemos aqui para mostrar que ela vale para $\Re \ s > 0$. Bastaria definir $f(s)$ para $\Re \ s > 0$ e usar que ela é nula sobre os reais positivos.
    ...
    \begin{proof}
        ...
        \]
        %
        Portanto, a transformada de Fourier de $g(x) = e^{-\pi tx^2}, t > 0$, é dada por
        %
        \begin{align*}
            \widehat{g}(\xi) &= \int_{-\infty}^{\infty} e^{-\pi tx^2}e^{-2\pi ix\xi} \, d\xi \\
                             &= \int_{-\infty}^{\infty} 
                             e^{-\pi u^2}e^{-2\pi i\frac{u}{\sqrt{t}}\xi} \frac{1}{\sqrt{t}} \, du \\
                             &= \frac{1}{\sqrt{t}}
                             \int_{-\infty}^{\infty} e^{-\pi u^2}e^{-2\pi iu\frac{\xi}{\sqrt{t}}} \, du \\
                             &= \frac{1}{\sqrt{t}}\exp\left( -\pi \frac{\xi^2}{t} \right),
        \end{align*}
        %
        onde introduzimos a substituição $u = \sqrt{t}x$. Portanto, segue da Fórmula da Soma
        de Poisson que
        %
        \begin{align*}
    ...
    \begin{proof}
        ...
        $C = 1 + 2\sum_{n=1}^{\infty} e^{-\pi n^2}$.
        
        Para o segundo item, basta notarmos que
        %
        \begin{align*}
    ...
    $\Gamma$, $\zeta$ e $\vartheta$. Um primeiro resultado nesse sentido é o seguinte.
    %
    \begin{teorema}
    ...
    %
    \begin{proof}
        {\red justificar o Fubini}
        
        Para provar a identidade desejada, vamos precisar da seguinte equação auxiliar:
        %
        \begin{equation}
        \label{eq-aux-zeta-gamma-teta}
            \int_0^{\infty} e^{-\pi n^2u}u^{s/2 - 1} \, du
            = \pi^{-s/2}\Gamma\left( \frac{s}{2} \right)\frac{1}{n^s}.
        \end{equation}
        %
        Para prová-la, basta considerar a mudança de variáveis $\pi n^2 u = t$ para obter
        %
        \begin{align*}
            \int_0^{\infty} e^{-\pi n^2u}u^{s/2 - 1} \, du 
        ...
        \end{align*}
        %
        Daí, somando a identidade sobre $n\in\N$ e usando o Teorema de Fubini, segue finalmente que
        %
        \begin{align*}
            \sum_{n\in\N} \int_0^{\infty} e^{-\pi n^2u}u^{s/2 - 1} \, du
            &= \pi^{-s/2}\Gamma\left( \frac{s}{2} \right)\sum_{n\in\N}\frac{1}{n^s} \\
            \int_0^{\infty} \left(\sum_{n\in\N} e^{-\pi n^2u}\right) u^{s/2 - 1} \, du 
            &= \pi^{-s/2}\Gamma\left( \frac{s}{2} \right)\zeta(s) \\
    ...
    \end{proof}
    %
    Esse último teorema merece alguns comentários. Observe, primeiramente, que o lado esquerdo
    da identidade contém uma função inteira, $\pi^{s/2}$, uma função meromorfa em $\C$ com
    polos simples nos inteiros pares não positivos, $\Gamma(s/2)$, e uma função holomorfa em
    $\Re(s) > 1$. Portanto, se definirmos $\xi:\{ s\in\C : \Re(s) > 1 \}\to\C$ por
    %
    \[
    \xi(s) = \pi^{-s/2}\Gamma\left( \frac{s}{2} \right)\zeta(s),
    \]
    %
    segue que ela é uma função holomorfa no semi-plano $\Re(s) > 1$.
    %
    \begin{teorema}
    \label{teo-form-reflexao-xi}
        A função $\xi:\{ s\in\C : \Re(s) > 1 \}\to\C$ admite uma continuação analítica a todo
        $\C$ como uma função meromorfa com polos simples em $s=0$ e $s=1$. Ademais, ela satisfaz
    ...
    \begin{proof}
        ...
        \]
        %
        Para calcular a parcela do lado esquerdo, usamos a equação funcional de $\psi$:
        %
        \begin{align*}
            \int_0^1 u^{s/2 - 1}\psi(u) \, du 
            &= \int_0^1 u^{s/2 - 1}\left[
            \frac{1}{\sqrt{u}}\psi\left(\frac{1}{u}\right) + \frac{1}{2\sqrt{u}} - \frac{1}{2}
            \right] \, du \\
            &= \int_0^1\left[
            u^{s/2 - 1}u^{-1/2}\psi\left(\frac{1}{u}\right) + \frac{u^{s/2 - 3/2}}{2} - \frac{u^{s/2 - 1}}{2}
            \right] \, du \\
        ...
        \end{align*}
        %
        Agora, vamos mostrar que a integral no lado direito define uma função holomorfa de $s$.
        Para tanto, defina
        %
        \[
        ...
        \]
        %
        Daí, dada uma curva de Jordan suave por partes $\y\subseteq\C$, temos
        %
        \begin{align*}
            \int_{\y} g_n(s) \, ds 
            &=\int_{\y} \left( 
        ...
        \]
        %
        e mostrar que as restrições convergem uniformemente para $g:S_{-M, M}\to\C$ dada por
        %
        \[
        g(s) = \int_1^{\infty} \left[ u^{s/2 - 1} + u^{-s/2 - 1/2} \right] \psi(u) \, du.
        \]
        %
        Já sabemos que $g$ está bem definida em $\Re(s) > 1$. Para mostrar que $g$ está bem
        definida em toda a faixa $S_{-M, M}$, basta mostrarmos que
        %
        \[
        ...
        \]
        %
        sendo $K = M/2 - 1/2$. Daí, notando que
        %
        \[
        |\vartheta(u) - 1| \leq \widetilde{C}e^{-\pi u} \iff |\psi(u)| \leq \widetilde{C'}e^{-\pi u},
        \]
        %
        segue que
        %
        \begin{align*}
        \int_1^{\infty} \left| u^{s/2 - 1} + u^{-s/2 - 1/2} \right|\cdot|\psi(u)| \, du
        &\leq \int_1^{\infty} \left( 2u^K\widetilde{C'}e^{-\pi u/2} \right)e^{-\pi u/2}\, du \\
        &\leq \widetilde{K}\int_1^{\infty} e^{-\pi u/2} \, du \\
        &< +\infty.
        \end{align*}
        %
        Usando estimativas idênticas às acima, podemos ver que
        %
        \begin{align*}
            |g_n(s) - g(s)| 
            &\leq \int_n^{\infty} \left| u^{s/2 - 1} + u^{-s/2 - 1/2} \right|\cdot|\psi(u)| \, du \\
            &\leq \widetilde{K}\int_n^{\infty} e^{-\pi u/2} \, du \\
            &= \widetilde{\widetilde{K}} e^{-\pi n/2} \xrightarrow{n\to\infty} 0.
        \end{align*}
        %
        Como $g_n$ é analítica e converge uniformemente para $g$ na faixa $S_{-M, M}$, segue que
        $g$ é analítica em $S_{-M, M}$. Por fim, como $M$ foi tomado arbitrariamente, segue que a
        aplicação
        %
        \[
        s \mapsto \int_1^{\infty} \left[ u^{s/2 - 1} + u^{-s/2 - 1/2} \right] \psi(u) \, du
        \]
        %
        ...
        \]
        %
        É imediato verificar que $\xi(s) = \xi(1-s)$:
        %
        \begin{align*}
    ...
    \begin{proof}
        ...
        %
        \[
        \zeta(s) = \pi^{s/2}\frac{1}{\Gamma(s/2)}\xi(s), \, \forall s\in\C, \, \Re(s) > 1.
        \]
        %
        Como $1/\Gamma$ é uma função inteira com zeros de multiplicidade 1 nos inteiros não positivos,
        segue que existe $g:\C\to\C$ holomorfa com $g(0)\neq 0$ tal que
        %
        \[
        \frac{1}{\Gamma(s)} = sg(s) \implies \frac{1}{\Gamma(s/2)} = \frac{s}{2}\cdot g(s/2).
        \]
        %
        Portanto,
        %
        \begin{align*}
            \zeta(s) &= \pi^{s/2}\frac{s}{2}g(s/2)\left( \frac{1}{s-1} - \frac{1}{s} + h(s) \right) \\
                     &= \frac{\pi^{s/2}}{2}\left( \frac{s}{s-1} - 1 + sh(s) \right)g(s/2),
        \end{align*}
        %
        sendo $h(s)$ a função holomorfa dada no teorema anterior. O lado direito da identidade acima
        é a extensão procurada, haja vista que define uma função holomorfa em $\C\setminus\{1\}$.
        Resta, portanto, mostrar que $s=1$ é polo simples e calcular seu resíduo. Para tanto,
        basta calcular
        %
        \begin{align*}
            \lim_{s\to 1} (s-1)\zeta(s) 
            &= \lim_{s\to 1} \frac{\pi^{s/2}}{2}\left( s - (s-1) + (s-1)sh(s) \right)g(s/2) \\
            &= \frac{\pi^{1/2}}{2}\lim_{s\to 1} sg(s/2) \\
            &= \frac{\pi^{1/2}}{2}\lim_{s\to 1} \frac{2}{\Gamma(s/2)} \\
            &= \frac{\pi^{1/2}}{2}\cdot\frac{2}{\Gamma(1/2)} \\
            &= \frac{\pi^{1/2}}{2}\cdot\frac{2}{\pi^{1/2}} \\
            &= 1.
        \end{align*}
        %
    \end{proof}
