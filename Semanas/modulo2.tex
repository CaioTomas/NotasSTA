...
o que encerra a prova do teorema.
    ...
    quando $R\to\infty$ e, depois, usar o Teorema dos Resíduos para lidar com o polo em $s=1$. Então, defina
    $F:\{s\in\C : \Re(s) > 0\}\setminus\{1\}\to\C$ por
    %
    \[
    F(s) = \frac{x^{s+1}}{s(s+1)}\left(-\frac{\zeta'(s)}{\zeta(s)}\right).
    \]
    %
    Observe que $F$ é holomorfa em seu domínio, que seu único polo é em $s=1$ e que
    %
    \[
    ...
    e $\beta_2$ vão a 0 quando $R\to\infty$. Para tanto, observe que se $s\in\beta_1$ então pelas
    ...
    \end{figure}
    %
    Para aplicar o Teorema dos Resíduos mais à frente, precisamos calcular o resíduo de $F$ em $s=1$,
    e isso envolve calcular o resíduo da derivada logarítmica de $\zeta$. Consideremos, então, $\e>0$ dado
    e $s\in D(1,\e)$. Vimos que no semi-plano $\Re(s) > 0$ podemos escrever
    %
    \[
    \zeta(s) - \frac{1}{s-1} = H(s),
    \]
    ...
    \]
    %
    Podemos dividir essa expressão por $\zeta(s)$, uma vez que $\zeta$ explode em $s=1$ e, portanto,
    no disco $D(1,\e)$ podemos garantir que $\zeta(s) \neq 0$ para $\e$ suficientemente pequeno.
    Isso nos dá
    %
    \begin{align*}
        \frac{\zeta'(s)}{\zeta(s)} &= \frac{H'(s) - \frac{1}{(s-1)^2}}{H(s) + \frac{1}{s-1}} \\
    ...
    %
    \[
    \res(F,1) = \lim_{s\to 1} (s-1)F(s) = -\lim_{s\to 1} \frac{-x^{s+1}}{s(s+1)} = \frac{x^2}{2}.
    \]
    %
    ...
    \begin{enumerate}[i)]
        ...
        $\mathcal{R} = [1-\delta,1+\delta]\times[-T,T]$ não tenhamos zeros de $\zeta$, ou seja, singularidades
        de $F$. 
    \end{enumerate}
    %
    ...
    \end{align*}
    %
    Dado $\e>0$, seja $T_0 = 16K_{1/2}^2/\e^2$. Tomando $T \geq T_0$ e fazendo a troca de variáveis
    $u = -t$, temos
    %
    \begin{align*}
    ...
    %
    \begin{align*}
        \left| \int_{\varphi_5} F(s) \, ds \right| &\leq \int_{\varphi_5} F(s) \, ds \\
                                              &= \int_T^{+\infty} 
                                              \frac{|x^{2 + it}|}{|(1 + it)(2 + it)|}
                                              \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt \\
                                              &\leq \int_T^{+\infty} 
