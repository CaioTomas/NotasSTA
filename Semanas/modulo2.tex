...
\section{A função Gama}
    ...
    Para todo $s\in\C$ tal que $\Re(s)>0$ temos $\Gamma(s+1) = s\Gamma(s)$.
    \end{teorema}
    ...
    Já que $\C \setminus \{0, -1, -2, \dots\}$ é um domínio segue do Principio da Identidade que $f$ é identicamente nula.
    \end{proof}
    
    \medskip 
    Esta equação funcional é bem conhecida para a função $\Gamma$ real. Usando este fato poderíamos ter procedido, como fizemos acima, para mostrar que esta identidade se estende para o semiplano $\Re(s)> 0$. Para isto bastaria definir $f(s)$ no semiplano $\Re(s)> 0$ e usar que ela é identicamente nula sobre os reais positivos.
    
    \medskip 
    
    Podemos obter a continuação da função $\Gamma$ de uma forma alternativa, decompondo em uma soma de duas parcelas sua representação integral como segue
    %
    \begin{equation}
    \label{eq-Gamma1}
    ...
    Por conta do princípio da identidade e pelo fato de continuações analíticas preservarem as equações funcionais (nesse caso), basta provar este resultado para $0<s<1$. 
    
    O primeiro passo da prova da equação funcional para $0<s<1$ é o lema a seguir.
    ...
    \begin{proof}
        ...
        Considere a curva $\y$ como no diagrama abaixo, onde $R>1$.
        %
        \begin{figure}[H]\centering
            \includegraphics{Figuras/curva gama.pdf}
        \end{figure}
        %
        Denote por $\y_1$ a curva com parametrização $R + 2\pi i t$ com $t \in [0,1]$. A integral de $f$ ao longo de $\y_1$ deve ir a $0$ quando $R \to \infty$. Basta observar que 
        %
        \begin{align*}
    ...
    De posse deste lema, estamos em condições de apresentar a demonstração do Teorema \ref{teo-gama-gamma-seno}.
    
    Usando a mudança de variáveis, $x = tu$ para algum $t>0$, podemos escrever
    %
    \begin{align*}
    ...
    \begin{align*}
        \Gamma(1-s)\Gamma(s) 
        &= \Gamma(1-s)\int_{0}^{\infty}e^{-t}t^{s-1}\, dt \\[0.3cm]
        %
        &= \int_{0}^{\infty}e^{-t}t^{s-1}\Gamma(1-s)\, dt \\[0.3cm]
        %
        &= \int_{0}^{\infty}e^{-t}t^{s-1}\int_{0}^{\infty}e^{-tu}u^{-s}t^{1-s}\, du \, dt \\[0.3cm]
        %
        &= \int_{0}^{\infty}\int_{0}^{\infty}e^{-t(1+u)}u^{-s}\, du \, dt \\[0.3cm]
        %
        &= \int_{0}^{\infty} u^{-s}\int_{0}^{\infty}e^{-t(1+u)}\, dt \, du \\[0.3cm]
        %
        &= \int_{0}^{\infty} u^{-s} \left [-\frac{e^{-t(1+u)}}{1+u} \right]_{t=0}^{\infty} \, du \\[0.3cm]
        %
        &= \int_{0}^{\infty} \frac{u^{-s}}{1+u}  \, du \\[0.3cm]
        %
    ...
    Observe agora, que a aplicação $s \mapsto 1-s$ pode ser vista como a reflexão em torno do ponto $1/2$. 
    Para todo $s \in \C$ temos $\frac{s + (1-s)}{2} = \frac{1}{2}$ e isto mostra que $1/2$ 
    é o ponto médio entre estes dois pontos. A função $\sen(\pi s)$ é bem conhecida 
    e sabemos calcular os seus valores facilmente. 
    Juntando isto ao teorema que acabamos de mostrar, concluímos que, 
    se sabemos calcular $\Gamma(s)$ no semiplano $\Re(s) > 1/2$, 
    então sabemos calculá-la também no semiplano $\Re(s) < 1/2$.
    
    Outra consequência deste teorema é sobre a função recíproca da função 
    $\Gamma(s)$, ou seja, $1/\Gamma(s)$. Podemos escrever
    %
    ...
    \end{align*}
    %
    Portanto, o argumento apresentado acima demonstra o seguinte teorema.
    %
    \begin{teorema}
    A aplicação $s\longmapsto 1/\Gamma(s)$ 
    pode ser estendida à uma função inteira que tem apenas zeros simples localizados em $s = 0,-1,-2, \dots$.
    \end{teorema}
    ...
    \end{figure}
    %
    Seja $\delta>1$ e considere o semi-plano $S_{\delta, +\infty}$ definido por
    %
    \[
    S_{\delta, +\infty} 
    \equiv 
    ...
    a função holomorfa definida pela seguinte expressão
    %
    \[
    ...
    Nosso próximo objetivo é mostrar que a função $\zeta$ admite uma extensão meromorfa à todo $\C$, com apenas
    ...
    a função teta de Jacobi $\vartheta: (0,\infty)\to\R$
    dada por
    %
    \begin{equation}
    ...
    \end{equation}
    %
    Veremos a seguir (Lema \ref{lema-eq-funcional}) que assim como $\Gamma$, a função $\vartheta$ também satisfaz uma equação funcional,
    a saber
    %
    \begin{equation}
    \label{eq-funcional-teta}
        \vartheta(t) = t^{-1/2}\vartheta\left( \frac{1}{t} \right), \qquad \forall t>0.
    \end{equation}
    %
    ...
    \begin{proof}
        ...
        Note que esta série {\bf não} converge para $u=0$\,!
        ...
        cada $\delta>0$ fixado. Consequentemente, como $\delta>0$ é arbitrário segue que a aplicação acima define uma função complexa contínua em $(0,+\infty)$ e portanto Riemann integrável em cada um dos intervalos limitados e fechados desta semi-reta.
...
o que encerra a prova do teorema.
    ...
    \begin{proof}
        ...
        Considere então a extensão meromorfa da função $\xi$ definida pelo lado direito da igualdade abaixo
        %
        \begin{align}
        \label{eq-xi-aux1-polos-holomorfa}
            \xi(s) 
            &= 
            \frac{1}{s-1} - \frac{1}{s} 
            + 
            \int_1^{\infty} 
            [u^{s/2 - 1} + u^{-s/2 - 1/2}]\psi(u) \, du.
            \nonumber\\[0.3cm]
            &\equiv 
            \frac{1}{s-1} - \frac{1}{s} + h(s),
        \end{align}
        %
        onde $h$ é a função inteira definida em 
        \eqref{eq-aux1-cont-analitica-xi}. 
        
        
        Notando que a soma das duas primeiras parcelas e o integrando 
        na igualdade acima são invariantes pela aplicação $s\longmapsto 1-s$ 
        podemos verificar  imediatamente que $\xi(s) = \xi(1-s)$. De fato, 
        %
        \begin{align*}
            \xi(1-s) 
            &= \frac{1}{1-s-1} - \frac{1}{1-s} 
    ...
    Com esse teorema, podemos finalmente apresentar o argumento que mostra que a função $\zeta$ admite uma extensão meromorfa à todo plano complexo, cuja a única singularidade é um polo simples localizado no ponto $s=1$. Além do mais, esta extensão tem  resíduo um nesta singularidade. 
    %
    \begin{teorema}
    \label{teo-cont-zeta}
        A função $\zeta:\{ s\in\C : \Re(s) > 1 \} \to \C$ admite uma continuação meromorfa a todo $\C$,
        com apenas um polo simples em $s=1$, além do mais $\res(\zeta,1)=1$.
    \end{teorema}
    %
    \begin{proof}
        Pela definição da função $\xi$, equação \eqref{def-fun-xi}, sabemos que para todo $s\in\C$ tal que $\Re(s) > 1$ temos
        %
        \[
        \xi(s) = \pi^{-s/2}\Gamma\left(\frac{s}{2}\right)\zeta(s).
        \]
        %
        Também sabemos que, neste semi-plano, $\Gamma(s/2)\neq 0$. Portanto, é lícito escrever
        %
        \begin{align}
        \label{eq-aux1-extensao-zeta}
        \zeta(s) 
        = 
        ...
        \]
        %
        Da observação acima e das identidades 
        \eqref{eq-aux1-extensao-zeta} 
        e 
        \eqref{eq-xi-aux1-polos-holomorfa}
        segue que para $\Re(s)>1$ temos
        %
        \begin{align*}
            \zeta(s) &= 
            \pi^{s/2}\frac{1}{\Gamma(s/2)}\xi(s)
        ...
        \end{align*}
        %
        onde $h$ é a função inteira definida em \eqref{eq-aux1-cont-analitica-xi}.
        
        O lado direito da identidade acima
        é a extensão de zeta desejada, 
        haja vista que esta expressão define uma função holomorfa em $\C\setminus\{1\}$.
        
        
        Resta, portanto, mostrar que $s=1$ é 
        polo simples e calcular seu resíduo. Para tanto,
        basta verificar que o seguinte limite existe e é um número complexo não-nulo
        %
        \begin{align*}
    ...
    em termos de um produto infinito.
    %
    ...
    \begin{proof}
        ...
        da série de Dirichlet de $\zeta$, ficamos com
        %
        \begin{equation}
    ...
    Nesta seção vamos determinar novas regiões do plano complexo nas quais a função zeta não se anula.
    
    
    Lembramos que o Teorema \ref{teo-form-prod-euler} nos garante que no semi-plano $\Re(s)>1$, temos $\zeta(s) \neq 0$.
    
    Já que 
    
    Da equação funcional para $\zeta$, sabemos que os inteiros pares não positivos,
    $\{0, -2, -4, -6, \dots\}$ são zeros dessa função, geralmente chamados
    ...
    \label{lema-aux-cresc-inv-gama}
        Seja $k>0$ fixado. Para todo $s\in\C$  satisfazendo $|s|\geqslant 1$ temos
        %
        \[
        e^{k|s|} \leq e^k \exp\Big(k\,|s|\,\ln|s|\Big).
        \]
        %
    \end{lema}
    %
    \begin{proof}
        Seja $k$ um número real positivo fixado. 
        Observe que para mostrar a validade do lema, basta mostrar que para todo $|s|\geqslant 1$ temos
        %
        \[
        \exp\Big(k\,|s|\,(1 - \ln|s|)\Big) \leq e^k.
        \]
        %
        Para verificar a validade desta desigualdade é suficiente mostrar que
        a função $f:[1,+\infty)\to\R$ definida por $f(x) = x(1-\ln x)$, para $x\geq 1$, tem um ponto de máximo global em $x=1$.
        
        Esta última afirmação, por sua vez, pode ser verificada observando que 
        %
        \[
        f'(x) = 1 - \ln x - x \left(\frac{1}{x}\right) = -\ln x \leq 0, 
        \qquad \forall x\geqslant 1.
        \]
        %
        Já que a desigualdade acima implica que $f$ é uma função monótona não-crescente e consequentemente que $x=1$ é um ponto de máximo global. Já que $f(1)=1$ segue da monotonicidade da função exponencial,
        da positividade de $k$ e da hipótese $|s|\geqslant 1$, que 
        $
        \exp\big(k|s|(1-\ln|s|)\big)
        =
        \exp(kf(|s|))
        \leqslant 
        \exp(kf(1))
    ...
    \label{teo-ordem-cresc-inv-gama}
        ...
        %
        \[
        \left| \frac{1}{\Gamma(s)} \right| \leq K_1 \exp\Big(K_2\, |s|\, \ln|s|\Big),
        \]
        %
        onde $K_1$ e $K_2$ são constantes reais positivas. Consequentemente, a função $1/\Gamma$ tem ordem de crescimento um, 
        isto é, para cada $\e>0$ existe uma constante positiva $C(\e)>0$ tal que
        %
        \[
        \left| \frac{1}{\Gamma(s)} \right| \leq C(\e)\exp\Big(K_2|s|^{1+\e}\Big),
        \]
        %
        para todo $s\in\C$.
    \end{teorema}
    ...
    \begin{proof}
        ...
        \[
        \Gamma(s)\Gamma(1-s) = \frac{\pi}{\sen(\pi s)}, 
        \qquad \forall s\in\C\setminus\Z;
        \]
        %
        e a seguinte representação de sua extensão meromorfa 
        (Teorema \ref{teo-cont-meromorfa-gamma2})
        %
        \[
        \Gamma(s) = \sum_{n=0}^{\infty} \frac{(-1)^n}{n!(n+s)} 
                    + \int_1^{\infty} e^{-t}t^{s-1} \, dt, \, 
                    \qquad\forall s\in\C\setminus\{0, -1, -2, \dots\}.
        \]
        %
        Usando as duas identidades acima, podemos afirmar que para todo $s\in\C\setminus\Z$ temos 
        a seguinte igualdade 
        %
        \begin{align}
        \label{eq-expressao-inv-gama}
            \frac{1}{\Gamma(s)} &= \Gamma(1-s)\frac{\sen(\pi s)}{\pi} \nonumber \\ 
                                ...
                                \frac{\sen(\pi s)}{\pi}
                                + \left( \int_1^{\infty} e^{-t}t^{-s} \, dt \right)
                                \frac{\sen(\pi s)}{\pi}.
        \end{align}
        %
        Para facilitar o argumento, vamos decompor a prova em duas partes. Na primeira parte, vamos mostrar como estimar a integral
        que aparece na segunda parcela de \eqref{eq-expressao-inv-gama}. Em seguida, vamos estimar a parcela correspondente a série que aparece na primeira parcela de \eqref{eq-expressao-inv-gama}.
        
        Para a primeira parte, vamos começar mostrando a validade da seguinte desigualdade
        %
        \begin{equation}
        \label{eq-cota-int-parte-real}
            \int_1^{\infty} e^{-t} t^{\sigma - 1} \, dt \leq \exp\Big((\sigma + 1)\ln(\sigma + 1)\Big),
        \end{equation}
        %
        sendo $\Re(s) = \sigma \geq 0$. De fato, escolha $n\in\N$ tal que 
        $\sigma \leq n\leq \sigma+1$. Então, usando o fato de que $0< 1/t \leq 1$ para $t\geqslant 1$ e também
        a monotonicidade do logaritmo e da exponencial, temos
        %
        \begin{align*}
            \int_1^{\infty} e^{-t}t^{\sigma - 1} \, dt 
            &\leq \int_1^{\infty} e^{-t}t^{\sigma} \, dt \\[0.3cm]
            &\leq \int_1^{\infty} e^{-t} t^n \, dt \\[0.3cm]
            &= \Gamma(n+1) \\[0.3cm]
            &= n! \\[0.3cm]
            &\leq n^n \\[0.3cm]
            &= \exp\Big(n\ln n\Big) \\[0.3cm]
            &\leq \exp\Big((\sigma + 1)\ln(\sigma + 1)\Big).
        \end{align*}
        %
        Essa desigualdade nos permite concluir que
        %
        \[
        \left| \int_1^{\infty} e^{-t} t^s \, dt \right| \leq \exp\Big((|\sigma|+1)\ln(|\sigma|+1)\Big), \, 
        \qquad \forall s\in\C.
        \]
        %
        Para majorar a segunda parcela que aparece na expressão \eqref{eq-expressao-inv-gama}, precisamos encontrar cotas superiores para a expressão 
        $\exp\big((|\sigma|+1)\ln(|\sigma|+1)\big)$. 
        Essa majoração será obtida analisando separadamente dois casos. 
        
        
        \medskip 
        
        Primeiro, vamos considerar o caso $0 < |s| < 1$.
        Antes de prosseguir, vale a pena mencionar que embora a maior preocupação no estudo de ordem de crescimento seja voltada para valores grandes do parâmetro $|s|$, a estimativa para valores pequenos deste parâmetro é sempre parte do trabalho.  
        
        \medskip 
        
        %
        Voltando à estimativa. No caso em que $0 < |s| < 1$ temos, das propriedades elementares do valor absoluto, da monotonicidade
        da função logaritmo natural e das propriedades elementares da função exponencial que 
        %
        \begin{align*}
            \exp\Big((|\sigma|+1)\ln(|\sigma|+1)\Big) 
                                            &\leq \exp\Big((|s|+1)\ln(|s|+1)\Big) \\[0.3cm]
                                            &= \exp\Big(|s|\ln(|s|+1)\Big) \exp\Big(\ln(|s|+1)\Big) \\[0.3cm]
                                            &\leq \exp\Big(|s|\ln(|s|+1)\Big) \exp\Big(\ln(1+1)\Big) \\[0.3cm]
                                            &= 2\exp\Big(|s|\ln(|s|+1)\Big) \\[0.3cm]
                                            &= 2\exp\left( |s|\ln\left( |s|\cdot \Big(1 + \frac{1}{|s|}\Big) \right) \right) \\[0.3cm]
                                            &\leq 2\exp\Big(|s|\ln|s|\Big)\exp\left(|s|\ln \Big(1 + \frac{1}{|s|}\Big)\right).
        \end{align*}
        %
        Observe que, pela Regra de L'Hôpital,
        %
        \begin{align*}
            \lim_{s\to 0} \left[ |s|\ln\left( 1 + \displaystyle\frac{1}{|s|} \right) \right]
            &= \lim_{s\to 0} 
            \frac{\ln \left(1 + \displaystyle\frac{1}{|s|}\right)}
            {\displaystyle \frac{1}{|s|} } 
            \\[0.5cm]
            &= \lim_{x\to 0^+} 
            \frac{\ln\left(1 + \displaystyle\frac{1}{x}\right)}
            {\displaystyle\frac{1}{x}} 
            \\[0.5cm]
            &= \lim_{x\to 0^+} 
            \frac{\left(1 + \displaystyle\frac{1}{x}\right)^{-1}
            \left(-\displaystyle\frac{1}{x^2}\right)}{-\displaystyle\frac{1}{x^2}}
            \\[0.5cm]
            &= 0.
        \end{align*}
        %
        Portanto, pela definição de limite, dado $\e=1$ sabemos que existe $0<\delta<1$ tal que para todo $0<|s|<\delta$ temos
        %
        \[
        \left| |s|\, \ln \left(1 + \displaystyle\frac{1}{|s|}\right) -0 \right| 
        < 1 
        \quad\implies \quad
        |s|\ln\left(1+\frac{1}{|s|}\right) \leq 2.
        \]
        %
        Voltando à estimativa, temos que para $0<|s|<1$ vale que
        %
        \begin{align*}
            \exp\Big((|\sigma|+1)\ln(|\sigma|+1)\Big) &\leq 2e^2 \exp\Big(|s|\ln|s|\Big).
        \end{align*}
        %
        ...
        \begin{align*}
            \exp\Big((|\sigma|+1)\ln(|\sigma|+1)\Big) 
                                            &\leq \exp\Big((|s|+1)\ln(|s|+1)\Big) \\[0.3cm]
                                            &\leq \exp\Big(2|s|\ln(2|s|)\Big) \\[0.3cm]
                                            &= \exp\Big(2|s|(\ln|s| + \ln 2)\Big) \\[0.3cm]
                                            &\leq \exp\Big(2|s|\ln|s|\Big) \exp\Big(2|s|\Big) \\[0.3cm]
                                            &\leq \exp\Big(2|s|\ln|s|\Big) e^2\exp\Big(2|s|\ln|s|\Big) \\[0.3cm]
                                            &\leq e^2 \exp\Big(4|s|\ln|s|\Big).
        \end{align*}
        %
        ...
        \begin{align*}
            ...
            &\leq C_3 \exp\Big(C_4|s|\ln|s|\Big) + C_1 \exp\Big(C_2|s|\ln|s|\Big) \\[0.3cm]
            &= K_1 \exp\Big(K_2|s|\ln|s|\Big),
        \end{align*}
        %
        sendo
        %
        \[
        K_1 = \frac{2e^{\pi + 2}}{\pi} \qquad \text{ e } \qquad K_2 = 4.
        \]
        %
    \end{proof}
    %
    ...
    \label{teo-form-prod-inv-gama}
        ...
        \]
        %
        é a conhecida constante de Euler-Mascheroni.
    ...
    \label{prop-2.7-Stein}
        ...
        com $0\leq\sigma_0\leq1$ e para cada $\e>0$ 
        existe uma uma constante $C_{\e}>0$ tal que
        %
        \begin{enumerate}[i)]
    ...
    \label{prop-cota-inv-zeta}
        Para cada $\e>0$ dado existe uma constante positiva $C_{\e}$ tal que %
        \[
        \frac{1}{|\zeta(s)|} \leq C_{\e}|t|^{\e},
        \]
        %
        para todo $s = \sigma + it$ com $\sigma\geq 1$ e $|t|\geq 1$.
    \end{proposicao}
    %
    \begin{proof}
        ...
        \label{eq-aux-estrela}
            ...
            = |\zeta(\sigma + it)|^4 
            &\geq 
            \frac{1}{|\zeta^3(\sigma)||\zeta(\sigma + 2it)|} 
            \nonumber\\
            &\geq 
            \frac{1}{C'(\sigma - 1)^{-3}C_{\e}|2t|^{\e}} 
            \nonumber\\
            &= 
        ...
        %
    \bigskip 
    Caso 1: existe $A>0$ tal que $A|t|^{-5\e} \leq \sigma - 1$. 
    ...
    %
    \begin{figure}[H]\centering
        \includegraphics{Figuras/psi de Chebyshev com descontinuidades.pdf}
    \end{figure} 
    %
    ...
    \begin{proof}
        ...
        \]
        %
        Logo, dividindo por $x$ e observando que 
        $\pi(x^{\alpha}) \leq x^{\alpha}$ para todo $x\geq 2$, temos
        %
        \begin{equation}
        \label{eq-aux-limsup}
    ...
    A proposição a seguir vai mostrar que o comportamento 
    assintótico de $\psi$ segue do comportamento
    assintótico de $\psi_1$.
    %
    \begin{proposicao}
    \label{prop-assint-psi1-implica-assint-psi}
        Supondo que $\psi_1(x) \sim x^2/2$ quando 
        $x\to\infty$, então $\psi(x)\sim x$ quando $x\to\infty$ e,
    ...
    \begin{proof}
        ...
        \]
        %
        e, tomando o limite quando $\alpha\to 1^-$, 
        obtemos o item ii) e fica demonstrada
        a proposição.
    \end{proof}
    %
    ...
    Daqui, o passo seguinte é relacionar $\psi$ e $\zeta$. 
    Essa relação surge da igualdade
    %
    \[
    ...
    \end{equation}
    %
    O comportamento assintótico $\psi_1(x)\sim x^2/2$ 
    será consequência via \eqref{eq-der-logaritmica-zeta-e-psi}
    da relação entre $\psi_1$ e $\zeta$, que tratamos a seguir. 
    Primeiro, enunciamos um lema
    um puramente técnico que será necessário mais à frente.
    %
    \begin{lema}
    \label{lema-integral-segmento}
        Dados $a,c>0$ quaisquer, temos
        %
        \[
        \frac{1}{2\pi i} \lim_{T\to\infty} 
        \int_{c - iT}^{c + iT} \frac{a^s}{s(s+1)} \, ds
        =
        \begin{cases}
            0,& \text{ se } 0 < a \leq 1; 
            \\[0.2cm]
            1 - \displaystyle\frac{1}{a},& \text{ se } 1 \leq a,
        \end{cases}
        \]
        %
        onde a integral é calculada ao longo do segmento de 
        reta que une os pontos $c-iT$ e $c+iT$.
    \end{lema}
    %
    \begin{proof}
        Observe que a variável de integração $s$ ao longo do segmento de reta unindo 
        $c+iT$ à $c+iT$ é da forma $s=c+it$ com $t\in [-T,T]$. Desta forma,
        para quaisquer $a,c>0$ dados, temos 
        %
        \begin{align*}
        |a^s| 
        = 
        ...
        %
        \begin{align}
        \label{eq-aux1-lema-aux-tchebyschev}
        \int_{b - iT}^{b + iT} \left|\frac{a^s}{s(s+1)}\right| \, |ds| 
        =
        \int_{-T}^T \frac{a^c}{|(c+it)(c+1+it)|} \, dt.
        \end{align}
        %
        Para poder tomar o limite, quando $T\to\infty$, na 
        igualdade acima basta observar que podemos cotar inferiormente 
        o denominador que a aparece na integral do lado direito 
        da igualdade acima como segue
        \[
        |(c+it)(c+1+it)|
        =
        (c^2+t^2)^{\frac{1}{2}}( (c+1)^2+t^2 )^{\frac{1}{2}}
        \geqslant
        c^2+t^2.
        \]
        Assim ficamos com a seguinte estimativa
        %
        \[
        \int_{-T}^T \frac{a^c}{|(c+it)(c+1+it)|} \, dt
        \leq
        \int_{-T}^T \frac{a^c}{c^2+t^2} \, dt.
        \]
        
        Da desigualdade acima e da identidade 
        \eqref{eq-aux1-lema-aux-tchebyschev} podemos
        concluir finalmente que existe o limite  
        \[
        \lim_{T\to\infty}
        \int_{b - iT}^{b + iT} \left|\frac{a^s}{s(s+1)}\right|\, |ds|
        \leqslant
        a^c\lim_{T\to\infty} 
        \int_{-T}^T \frac{1}{t^2+c^2} \, dt
        =
        2a^c\lim_{T\to\infty} 
        \int_{0}^T \frac{1}{t^2+c^2} \, dt
        =
        \frac{a^c\pi}{c}.
        \]
        %
        Portanto, existe o limite da integral que aparece no 
        enunciado do lema.
        
        Agora que sabemos que a integral imprópria do enunciado do lema existe, 
        vamos obter seu valor exato. Para isto vamos considerar 
        a função complexa auxiliar $f:\C\setminus\{-1,0\}\to\C$ 
        dada por
        %
        \[
        f(s) = \frac{a^s}{s(s+1)}. 
        ...
        O cálculo da integral será feito analisando-se separadamente os seguintes dois casos. 
        ...
        No que segue consideramos $T>c+1$ e o caminho suave por partes mostrado na figura abaixo.
        %
        \begin{figure}[H]\centering
            \includegraphics{Figuras/disco no SPE.pdf}
        \end{figure}
        %
        Como estamos assumindo que $T>1+c$, se $\Gamma(T)$ denota o caminho
        suave por partes mostrado na figura acima, podemos aplicar o Teorema dos Resíduos para verificar que
        %
        \[
        \frac{1}{2\pi i}\int_{\Gamma(T)} f(s) \, ds = 1 - \frac{1}{a}.
        \]
        %
        Por outro lado, segue das propriedades elementares da 
        integral complexa que
        %
        \[
        ...
        \]
        %
        sendo $C(T)$ o semi-circulo que aparece na figura acima, que pode ser pensado também como o traço do caminho $t\longmapsto c + Te^{it}$ com $t \in [\pi/2, 3\pi/2]$. 
        
        
        Vamos mostrar que a integral ao longo do caminho $C(T)$ tende a zero, quando $T\to\infty$. Para isto é suficiente mostrar que
        %
        \[
        ...
        %
        \begin{figure}[H]\centering
            \includegraphics{Figuras/disco no SPD.pdf}
        \end{figure}
        %
    ...
    \begin{proof}
        ...
        \begin{tabular}{%
            ...
            \]
        	&
        	\includegraphics{Figuras/função indicadora.pdf} 
        \end{tabular}
    ...
    Agora, vamos mostrar, finalmente, o comportamento assintótico de $\psi_1$. Para tanto, fixe $c>1$
    e considere $x\geq 2$. Observe o contorno abaixo, onde $R>T>1$.
    %
    \begin{figure}[H]\centering
        \includegraphics{Figuras/de beta para phi.pdf}
    \end{figure}
    %
    Vamos calcular a integral da Proposição \ref{prop-psi1-zeta} utilizando um argumento de deformação:
    a nossa estratégia será mostrar que podemos deformar o contorno da direita no contorno mais à esquerda
    quando $R\to\infty$ e, depois, usar o Teorema dos Resíduos para lidar com o polo em $s=1$. 
    
    Seja
    $F:\{s\in\C : \Re(s) > 0\}\setminus\{1\}\to\C$ 
    %
    a função complexa dada por
    \[
    F(s) = \frac{x^{s+1}}{s(s+1)}\left(-\frac{\zeta'(s)}{\zeta(s)}\right).
    \]
    %
    Observe que $F$ é holomorfa em seu domínio, que 
    sua única singularidade ocorre em $s=1$. 
    Portanto podemos aplicar o Teorema de Cauchy-Goursat 
    para obter a seguinte igualdade
    %
    \[
    \int_{\beta} F(s) \, ds + \int_{\beta_1} F(s) \, ds + \int_{\beta_2} F(s) \, ds = \int_{\phi} F(s) \, ds,
    \]
    %
    sendo $\phi = \phi_1 * \phi_2 * \phi_3 * \phi_4 * \phi_5$. Vamos mostrar que as integrais em $\beta_1$
    e $\beta_2$ vão a 0 quando $R\to\infty$. 
    
    
    Para tanto, observe que se $s\in\beta_1$ então pelas
    Proposições \ref{prop-2.7-Stein} e \ref{prop-cota-inv-zeta} temos
    %
    \[
    \left| \frac{\zeta'(s)}{\zeta(s)} \right| \leq K_{\eta} R^{\eta},
    \]
    %
    para $\eta > 0$ qualquer. Ora, então como $s = \sigma + iR$ temos
    %
    \begin{align*}
        |F(s)| &\leq K_{\eta}R^{\eta} \frac{|x^{\sigma + 1 + iR}|}{|\sigma + iR||\sigma + 1 + iR|} \\
               &\leq K_{\eta}R^{\eta} x^{\sigma + 1}R^{-2} \\
               &\leq K_{\eta}R^{\eta - 2}x^{c + 1}.
    \end{align*}
    %
    Daí, segue que
    %
    \[
    \left| \int_{\beta_1} F(s) \, ds \right| \leq \frac{K_{\eta}(c-1)x^{c+1}}{R^{2 - \eta}} \xrightarrow{R\to\infty} 0.
    \]
    %
    Analogamente, se $s\in\beta_2$ temos a mesma cota para a derivada logarítmica
    %
    \[
    \left| \frac{\zeta'(s)}{\zeta(s)} \right| \leq K_{\eta} R^{\eta}
    \]
    %
    e, como $s = \sigma - iR$, temos
    %
    \begin{align*}
        |F(s)| &\leq K_{\eta}R^{\eta} \frac{|x^{\sigma + 1 - iR}|}{|\sigma - iR||\sigma + 1 - iR|} \\
               &\leq K_{\eta}R^{\eta} x^{\sigma + 1}R^{-2} \\
    ...
    %
    \[
    \left| \int_{\beta_2} F(s) \, ds \right| \leq \frac{K_{\eta}(c-1)x^{c+1}}{R^{2 - \eta}} \xrightarrow{R\to\infty} 0.
    \]
    %
    Agora, vamos querer considerar o caminho $\phi$ do contorno acima juntamente com o caminho 
    $\varphi = \varphi_1 * \varphi_2 * \varphi_3 * var\phi_4 * \varphi_5$,
    ilustrado abaixo.
    %
    \begin{figure}[H]\centering
        \includegraphics{Figuras/de phi para varphi.pdf}
    \end{figure}
    %
    Para aplicar o Teorema dos Resíduos mais à frente, 
    precisamos calcular o resíduo de $F$ em $s=1$,
    e isso envolve calcular o resíduo da 
    derivada logarítmica de $\zeta$ em $s=1$. 
    Seja $\e>0$ dado e $s\in D(1,\e)$. 
    Usando o Corolário \ref{cor-zeta-menos-singularidade} 
    sabemos que para todo $s$ pertencente ao semi-plano $\Re(s) > 0$, temos
    %
    \[
    \zeta(s) - \frac{1}{s-1} = H(s),
    \]
    %
    onde $H$ é uma função holomorfa nesse semi-plano. Derivando esta expressão com respeito a $s$, temos
    %
    \[
    \zeta'(s) = H'(s) - \frac{1}{(s-1)^2}.
    \]
    %
    Podemos dividir essa expressão por $\zeta(s)$, uma vez que $|\zeta|$
    fica arbitrariamente grande para valores de $s$ suficientemente 
    próximos de $s=1$. 
    Portanto, no disco $D(1,\e)$ podemos garantir que 
    $\zeta(s) \neq 0$, para $\e$ suficientemente pequeno.
    Isso nos dá
    %
    \begin{align*}
        \frac{\zeta'(s)}{\zeta(s)} 
        &= 
        \frac{H'(s) - \displaystyle\frac{1}{(s-1)^2}}
        {H(s) +\displaystyle \frac{1}{s-1}} 
        \\[0.4cm]
        &= 
        \frac{H'(s)(s-1)^2 - 1}{H(s)(s-1) + 1}\cdot\frac{1}{s-1}.
    \end{align*}
    %
    Daí, segue que
    %
    \[
    \lim_{s\to 1} (s-1)\frac{\zeta'(s)}{\zeta(s)} = \lim_{s\to 1} \frac{H'(s)(s-1)^2 - 1}{H(s)(s-1) + 1} = -1.
    \]
    %
    ...
    &=
    \lim_{s\to 1} (s-1)
    \frac{x^{s+1}}{s(s+1)}\left(-\frac{\zeta'(s)}{\zeta(s)}\right)
    \\[0.3cm]
    &=
    \lim_{s\to 1}\frac{x^{s+1}}{s(s+1)}
    \ \cdot \ 
    \lim_{s\to 1}(s-1)\left(-\frac{\zeta'(s)}{\zeta(s)}\right)
    \\[0.3cm]
    &= 
    \frac{x^2}{2}.
    \end{align*}
    %
    Agora, o próximo passo é escolher $T$ para, em seguida, escolhermos $\delta$. Isso se justifica
    porque vamos querer integrar $F$ em um retângulo aplicando o Teorema dos Resíduos e, para isso,
    precisamos garantir que a única singularidade de $F$ seja $s=1$. Faremos isto em dois passos:
    %
    \begin{enumerate}[i)]
        \item escolhemos $T$ suficientemente grande de modo que as integrais em $\varphi_1$ e $\varphi_5$ 
        fiquem pequenas;
        \item escolhido $T$, podemos escolher $\delta$ suficientemente pequeno de maneira que no retângulo
        $\mathcal{R} \equiv [1-\delta,1+\delta]\times[-T,T]$ 
        seja possível garantir que não temos nenhum zero de $\zeta$, ou seja, singularidades de $F$. 
    \end{enumerate}
    %
    Esses dois passos são necessários porque sabemos que $\zeta$ tem um número finito de zeros em qualquer compacto.
    Em particular, $\zeta$ tem um número finito de zeros em qualquer retângulo. Portanto, definida a ``altura''
    do retângulo (isto é, escolhido $T$), sabemos que $\zeta$ tem, digamos, $n$ zeros no retângulo 
    $[0,1]\times[-T,T]$. Daí, se $d$ é a menor distância desses zeros à reta $\Re(s) = 1$, 
    podemos escolher $\delta = d/2$.
    Feitas estas observações, mãos à obra.
    
    Observe que
    %
    \begin{align*}
        \left| \int_{\varphi_1} F(s) \, ds \right| &\leq \int_{\varphi_1} F(s) \, ds \\
                                              &= \int_{-\infty}^{-T} 
                                              \frac{|x^{2 + it}|}{|(1 + it)(2 + it)|}
                                              \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt \\
                                              &\leq \int_{-\infty}^{-T} 
                                              \frac{x^2}{t^2}
                                              \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt.
    \end{align*}
    %
    Sendo $s = 1 + it$, podemos tomar a cota
    %
    \[
    \left|\frac{\zeta'(s)}{\zeta(s)}\right| \leq K_{1/2}|t|^{1/2}.
    \]
    %
    Daí, temos
    %
    \begin{align*}
        \int_{-\infty}^{-T} \frac{x^2}{t^2} \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt
        &\leq K_{1/2}x^2\int_{-\infty}^{-T} |t|^{-3/2} \, dt.
    \end{align*}
    %
    Dado $\e>0$, seja $T_0 = 16K_{1/2}^2/\e^2$. 
    Tomando $T \geq T_0$ e fazendo a troca de variáveis
    $u = -t$, temos
    %
    \begin{align*}
        \left| \int_{\varphi_1} F(s) \, ds \right| &\leq K_{1/2}x^2\int_{-\infty}^{-T} |t|^{-3/2} \, dt \\
                                                   &= K_{1/2}x^2 \frac{u^{-1/2}}{-1/2}\Bigg|_T^{\infty} \\
                                                   &= 2K_{1/2}x^2\frac{1}{T^{1/2}} \\
                                                   &\leq \frac{\e}{2}x^2.
    \end{align*}
    %
    Analogamente, para $\varphi_5$ temos
    %
    \begin{align*}
    \left| \int_{\varphi_5} F(s) \, ds \right| 
    &\leq \int_{\varphi_5} |F(s)| \, |ds| \\
    &= \int_T^{+\infty} 
    \frac{|x^{2 + it}|}{|(1 + it)(2 + it)|}
    \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt \\
    &\leq \int_T^{+\infty} 
    ...
    \left|\frac{\zeta'(s)}{\zeta(s)}\right| \, dt \\
    &\leq K_{1/2}x^2\int_{-\infty}^{-T} |t|^{-3/2} \, dt \\
    &= K_{1/2}x^2 \frac{t^{-1/2}}{-1/2}\Bigg|_T^{\infty} \\
    &= 2K_{1/2}x^2\frac{1}{T^{1/2}} \\
    &\leq \frac{\e}{2}x^2.
    \end{align*}
    %
    Agora, note que sendo $\mathcal{R} = [1-\delta,1+\delta]\times[-T,T]$, temos pelo Teorema dos Resíduos que
    %
    \begin{align*}
        \int_{\phi} F(s) \, ds - \int_{\varphi} F(s) \, ds &= \int_{\mathcal{R}} F(s) \, ds \\
                                                           &= 2\pi i\res(F,1) \\
                                                           &= 2\pi i\frac{x^2}{2}.
    \end{align*}
    %
    ...
    %
    \[
    \psi_1(x) = \frac{1}{2\pi i} \int_{\phi} F(s) \, ds = \frac{x^2}{2} + \frac{1}{2\pi i}\int_{\varphi} F(s) \, ds.
    \]
    %
    Até o momento, temos
    %
    \begin{align*}
        \int_{\varphi} F(s) \, ds &= \int_{\varphi_1} F(s) \, ds + \int_{\varphi_5} F(s) \, ds 
                                    + \int_{\varphi_2*\varphi_3*\varphi_4} F(s) \, ds \\
                                  &\leq \e x^2 + \int_{\varphi_2*\varphi_3*\varphi_4} F(s) \, ds.
    \end{align*}
    %
    O passo final é lidar com a integral de $F$ em $\varphi_2$, $\varphi_3$ e $\varphi_4$. Começamos com
    $\varphi_2$. Observe que
    %
    \begin{align*}
        \left| \int_{\varphi_2} F(s) \, ds \right| 
        &\leq -\int_1^{1-\delta} \frac{|x^{-u + 3 - \delta - iT}|}{|(-u+3-\delta-iT)(-u+4-\delta - iT)|}K_{1/2}|T|^{1/2} \, du \\
        &\leq \frac{K_{1/2}|T|^{1/2}}{T^2} \int_{1-\delta}^1 x^{-u+3-\delta} \, du \\
        &= \widetilde{K} \int_{1-\delta}^1 e^{(-u+3-\delta) \ln x} \, du \\
        &= \widetilde{K} \frac{1}{\ln x}(x^2 - x^{2 - \delta}).
    \end{align*}
    %
    De maneira análoga,
    %
    \begin{align*}
        \left| \int_{\varphi_4} F(s) \, ds \right| 
        &\leq \int_{1-\delta}^1 \frac{|x^{u + 1 + iT}|}{|(u + iT)(u + 1 + iT)|}K_{1/2}|T|^{1/2} \, du \\
        &\leq \frac{K_{1/2}|T|^{1/2}}{T^2} \int_{1-\delta}^1 x^{u+1} \, du \\
        &= \widetilde{K} \int_{1-\delta}^1 e^{(u+1) \ln x} \, du \\
        &= \widetilde{K} \frac{1}{\ln x}(x^2 - x^{2 - \delta}).
    \end{align*}
    %
    {\red Por fim,}
    %
    \begin{align*}
        \left| \int_{\varphi_3} F(s) \, ds \right|
        &\leq \int_{-T}^T \frac{|x^{1 - \delta + 1 + it}|}{|(1 - \delta + it)(2 - \delta + it)|} K_{1/2}|T|^{1/2} \, dt \\
        &\leq K_{1/2}|T|^{1/2} x^{2 - \delta} \int_{-T}^T \frac{1}{|(1-\delta)||(2-\delta)|} \, dt \\
        &= \frac{2\widetilde{K}T^3}{|(1-\delta)||(2-\delta)|} x^{2 - \delta}.
    \end{align*}
    %
    {\red Aqui eu cotei pelo módulo da parte real porque a integral $\int_{-T}^T 1/t^2 \, dt$ não é bem definida}.
    
    Consequentemente,
    %
    \begin{align*}
        \left| \psi_1(x) - \frac{x^2}{2} \right| &= \frac{1}{2\pi}\left| \int_{\varphi} F(s) \, ds \right| \\
                                                 &\leq \frac{1}{2\pi}\left[ \e x^2 + 2\widetilde{K}\frac{x^2}{\ln x}
                                                 + \left(-2\frac{\widetilde{K}}{\ln x} 
                                                 + \frac{2\widetilde{K}T^3}{|(1-\delta)||(2-\delta)|}\right)x^{2-\delta}
                                                 \right]
    \end{align*}
    %
    e, portanto,
    %
    \begin{align*}
        \left| \frac{\psi_1(x)}{x^2/2} - 1 \right| &\leq \frac{1}{2\pi}\left[ 2\e + 4\frac{\widetilde{K}}{\ln x}
                                                 + 2\left(-2\frac{\widetilde{K}}{\ln x} 
                                                 + \frac{2\widetilde{K}T^3}{|(1-\delta)||(2-\delta)|}\right)\frac{1}{x^{\delta}}
                                                 \right] \\
                                                 &\leq \e + 2\frac{\widetilde{K}}{\ln x}
                                                 + \left(-2\frac{\widetilde{K}}{\ln x} 
                                                 + \frac{2\widetilde{K}T^3}{|(1-\delta)||(2-\delta)|}\right)\frac{1}{x^{\delta}}\\
                                                 &{\red \leq 4\e}
    \end{align*}
    %
    {\red para $x$ suficientemente grande, concluindo a prova do Teorema do Número Primo.}