...
\section{A função Gama}
    ...
    \begin{proof}
       ...
       \label{eq-aux2-lema-int-sum-teta}
        ...
        por uma aplicação direta do teste da raíz $n$-ésima. 
        Além do mais, segue do teste M de Weierstrass que para cada $s$ fixado que a aplicação 
        \[
        ...
        Próximo passo na prova do lema é mostrar que para cada $s\in \C$, com $\Re(s)>1$ fixado, o limite abaixo existe
        \[
        \lim_{N\to\infty}
    ...
    \begin{proof}
        ...
        \end{equation}
        %
        Para prová-la, basta considerar a mudança 
        de variáveis $\pi n^2 u = t$ e observar que
        %
        Somando a identidade acima sobre $n\in\N$ e 
        mudando a ordem da soma e de integração, segue finalmente que
        %
        \[
        \begin{array}{c}
            \displaystyle
            ...
            = 
            \displaystyle\pi^{-s/2}\Gamma\left( \frac{s}{2} \right)\zeta(s) 
            \\[0.5cm]
            \big\Downarrow
            \\[0.5cm]
            \displaystyle
    ...
    uma função inteira, $\pi^{s/2}$; 
    uma função meromorfa em $\C$ com
    polos simples nos inteiros pares não positivos, $\Gamma(s/2)$; 
    e uma função holomorfa em $\Re(s) > 1$. 
    
    Portanto a função $\xi:\{ s\in\C : \Re(s) > 1 \}\to\C$ dada por
    %
    \[
    ...
    \label{teo-form-reflexao-xi}
        A função $\xi:\{ s\in\C : \Re(s) > 1 \}\to\C$ 
        admite uma continuação meromorfa em $\C$, 
        com polos simples em $s=0$ e $s=1$. 
    ...
    \index{Fórmula!do Produto de Euler}
        Para todo $s\in\C$ tal que $\Re(s) > 1$, vale
        %
        \[
        \zeta(s) = \sum_{n=1}^{\infty} \frac{1}{n^s}
                 = \prod_{p\in\mathbb{P}} \left(
                 1 - \frac{1}{p^s} \right)^{-1}
        \]
        %
        e, além disso, $\zeta(s)\neq 0, \, \forall s\in\C$ tal que 
        $\Re(s) > 1$.
    \end{teorema}
    %
    \begin{proof}
        ...
        em série de Dirichlet de $\zeta$, temos
        %
        \begin{equation}
        ...
        \end{equation}
        %
        Antes de seguir para o próximo passo, é importante frisar um
        ponto: a priori, se $A$ é um conjunto infinito enumerável qualquer,
        então o símbolo
        %
        \[
        \sum_{n\in A} a_n
        \]
        %
        não está bem definido. De fato, se essa série for condicionalmente
        convergente, então o símbolo acima poderia tomar qualquer valor
        real, bastando apenas somar os termos em uma ordem adequada.
        Por outro lado, se a série convergir absolutamente, então a ordem
        em que somarmos não será relevante e, nesse caso, o símbolo acima
        está bem definido.
        No nosso caso, todas as séries são absolutamente convergentes e,
        portanto, usaremos essa notação.
        
        A ideia da demonstração é continuar esse processo, por
        indução, para os demais primos. Para tanto, vamos denotar
        %
        \[
        p\N = \{ p, 2p, 3p, \dots \}
        \]
        ...
        \]
        %
        sendo $p_n$ o $n$-ésimo primo.
        Daí, podemos reescrever \eqref{eq:zeta-sem-pares} como
        %
        \[
        ...
        \begin{align*}
            ...
            \\[0.5cm]
            \left|
            \prod_{j=1}^n \left(1 - \frac{1}{p_j^s}\right) - 1
            \right|
            = \left|
            \sum_{n\in\N(2,3, \dots, p_n)} \frac{1}{n^s}
            \right|
            &\leq \sum_{j=p_n}^{\infty} \left|\frac{1}{j^s}\right|,
            \, \forall n\geq 1.
        \end{align*}
        %
        Como a cauda da série de Dirichlet de $\zeta$ converge para $0$,
        segue que
        %
        \[
        \left[
        ...
        \]
        %
        Segue dessa igualdade que $\zeta(s)\neq 0$ no semi-plano
        $\Re(s) > 1$. Por fim, lembrando que se uma sequência
        $(b_n)_{n\in\N}\subseteq\C$ é tal que $b_n\to b\neq 0$
        então $1/b_n \to 1/b$, podemos reescrever a identidade
        acima como
        %
        \[
        \zeta(s) 
