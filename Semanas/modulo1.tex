...
\label{sec:aprox-runge}
    ...
    Convergência uniforme significa que, dado qualquer $\varepsilon > 0$, existe $N \in \N$ 
    tal que 
    %
    \begin{equation*}
    ...
    ou seja, escolhido qualquer $\varepsilon > 0$, conseguimos encontrar um polinômio $p_n$ 
    tal que $f(x)$ está $\varepsilon-$próximo de $p_n(x)$ para todo $x$ em $[a,b]$. 
    Se não fosse uniforme, poderia ocorrer que, para cada $\varepsilon > 0$, 
    ...
    \label{LR1}
        ...
        um compacto. Existem finitos segmentos de reta $\gamma_1, \dots, \gamma_N$ em 
        $\Omega - K$ tais que
        %
        \begin{equation*}
        f(z) 
        = 
        \sum^{N}_{j=1}\frac{1}{2 \pi i}\int_{\gamma_j}\frac{f(w)}{w-z} \, dw  \, , \forall z \in K.
        \end{equation*}
        %
    \end{lema}
    %
    \begin{proof}
        ...
        coordenados (veja Figura \ref{}). Seja $\mathcal{Q} = \{Q_1, \dots, Q_M\}$ o conjunto 
        ...
        %
        \begin{center}
            {\bf diagrama reticulado Stein}
        \end{center}
        %
        Sejam $\gamma_j$, com $j \in \{1, \dots, N\}$, os lados dos quadrados em $\mathcal{Q}$ 
        ...
        logo $\gamma_j\subset\Omega$ para todo $j$.
    ...
    \label{LR2}
        ...
        de funções racionais com singularidades em $\gamma$ que aproxima
        %
        \begin{equation*}
    ...
    %
    \begin{proof}
        Seja $\gamma : [0,1] \to \C$ um segmento de reta conforme o teorema e denote
        %
        \begin{equation*}
        ...
        \end{equation*}
        %
        Fixado $z \in K$ e dado $\varepsilon > 0$, existe um $\delta_z > 0$ tal que
        %
        \begin{equation}
        \label{ER1}
    ...
    $\partial \mathbb{D}$, o bordo do disco unitário  $\mathbb{D}$. Vamos mostrar que $f$ não
    ...
    $\partial \mathbb{D}$. Então existe uma sequência $\{p_n\}$ de polinômios tal que, 
    dado $\varepsilon > 0$, existe $N \in \N$ tal que
    %
    \begin{equation*}
    ...
    %
    \begin{align*}
        \left|\int_{\partial \mathbb{D}} f(z) \, dz\right| 
        &= \left|\int_{\partial \mathbb{D}} f(z) \, dz - \int_{\partial \mathbb{D}} p_n(z) \, dz\right| \\
        &= \left|\int_{\partial \mathbb{D}} f(z) - p_n(z)\, dz\right| \\
        &\leq \int_{\partial \mathbb{D}}|f(z) - p_n(z)| \, dz \xrightarrow{n\to\infty} 0,
    \end{align*}
    %
    ...
    \begin{proof}
        ...
        Como $K^c$ é conexo, existe uma curva $\gamma : [0,1] \to \C$ inteiramente contida em 
        $K^c$ tal que $\gamma(0) = z_0$ e $\gamma(1) =  z_1$. Defina $\rho = \frac{1}{2}d(K,\gamma)$;
        devemos ter $\rho > 0$ pois ambos $K$ e $\gamma$ são compactos. 
        Seja $\{w_0, w_1, \dots, w_l\}$ uma sequência finita de pontos em $\gamma$ com $w_0 = z_0$,
